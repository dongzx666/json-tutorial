...
void lept_copy(lept_value *dst, const lept_value *src) {
  ...
  case LEPT_ARRAY:
    ...
    memcpy(dst->u.a.e, src->u.a.e, src->u.a.size * sizeof(lept_value));
    break;
  case LEPT_OBJECT:
    lept_set_object(dst, src->u.o.capacity);
    dst->u.o.size = src->u.o.size;
    /**memcpy(dst->u.o.m, src->u.o.m, src->u.o.size * sizeof(lept_member));*/
    for (i = 0; i != dst->u.o.size; ++i) {
      lept_member *dst_mem = &dst->u.o.m[i];
      lept_member *src_mem = &src->u.o.m[i];
      dst_mem->klen = src_mem->klen;
      dst_mem->k = (char *)malloc(dst_mem->klen+ 1);
      memcpy(dst_mem->k, src_mem->k, dst_mem->klen+ 1);
      lept_copy(&dst_mem->v, &src_mem->v);
    }
    break;
  default:
    lept_free(dst);
...
void lept_set_string(lept_value *v, const char *s, size_t len) {
  ...
  lept_free(v);
  /* TODO(fengyu): valgrind memory leak [04-11-21] */
  /*https://github.com/miloyip/json-tutorial/issues/157*/
...
lept_value *lept_insert_array_element(lept_value *v, size_t index) {
  lept_value* last = NULL;
  /**size_t i;*/
  assert(v != NULL && v->type == LEPT_ARRAY && index <= v->u.a.size);
  last = lept_pushback_array_element(v);
  while (last != &v->u.a.e[index]) {
    lept_move(last, last-1);
    last--;
  }
  /**for (i = v->u.a.size - 2; i >= index; ++i) {*/
    /**memcpy(&v->u.a.e[i + 1], &v->u.a.e[i], sizeof(lept_value));*/
  /**}*/
  return &v->u.a.e[index];
}
...
void lept_erase_array_element(lept_value *v, size_t index, size_t count) {
  ...
  /*todo*/
  /**for (i = 0; i < v->u.a.size - count; ++i) {*/
    /**memcpy(&v->u.a.e[index + i], &v->u.a.e[index + count + i],*/
           /**sizeof(lept_value));*/
  /**}*/
  ...
  while (index + count < v->u.a.size) {
    lept_move(&v->u.a.e[index], &v->u.a.e[index+count]);
    ++index;
  }
  v->u.a.size -= count;
}
...
lept_value *lept_set_object_value(lept_value *v, const char *key, size_t klen) {
  lept_value* target = NULL;
  lept_member* temp = NULL;
  assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
  target = lept_find_object_value(v, key, klen);
  if (target != NULL) {
    return target;
  }
  ...
  temp = &v->u.o.m[v->u.o.size++];
  temp->k = (char*)malloc(klen+1);
  memcpy(temp->k, key, klen);
  temp->klen = klen;
  temp->k[klen] = '\0';
  lept_init(&temp->v);
  return &temp->v;
